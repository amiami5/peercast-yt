...
**移行は終わりました。** C++ のコードはすべて取り除き、ビルドはリポジトリの
...
* HTML と JavaScript。(Python の CGI スクリプト (`ui/cgi-bin`) とビルド時の Ruby は、C++ を取り除いたあとで Rust にした。)
