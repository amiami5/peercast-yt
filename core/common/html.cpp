...
void HTML::addHead()
{
    ...
    startNode("head");
        ...
        {
            snprintf(buf, sizeof(buf), "meta http-equiv=\"refresh\" content=\"%d;URL=%s\"", refresh, cgi::escape_html(refreshURL).c_str());
            startTagEnd(buf);
        }else if (refresh)
        {
            snprintf(buf, sizeof(buf), "meta http-equiv=\"refresh\" content=\"%d\"", refresh);
            startTagEnd(buf);
        }
    end();
}
...
void HTML::addLink(const char *url, const char *text, bool toblank)
{
    ...
    snprintf(buf, sizeof(buf), "a href=\"%s\" %s", url, toblank?"target=\"_blank\"":"");
    startNode(buf, text);
    end();
}
