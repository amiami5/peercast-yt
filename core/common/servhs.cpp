...
void Servent::CMD_login(const char* cmd, HTTP& http, String& jumpStr)
{
    ...
    if (servMgr->cookieList.neverExpire)
        http.writeLineF("%s %d_id=%s; path=/; expires=\"Mon, 01-Jan-3000 00:00:00 GMT\"; SameSite=Strict", HTTP_HS_SETCOOKIE, (int) servMgr->serverHost.port, idstr);
    else
        http.writeLineF("%s %d_id=%s; path=/; SameSite=Strict", HTTP_HS_SETCOOKIE, (int) servMgr->serverHost.port, idstr);
